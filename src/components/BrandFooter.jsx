import { FEEDBACK_URL, WALKTHROUGH, hasWalkthrough } from "../lib/app-links";
...
export default function BrandFooter({ version, buildDate }) {
  return (
    <footer
      role="contentinfo"
      className="mt-10 w-full border-t border-border pt-6"
    >
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-status-fresh">
              Nothing is stored
            </p>
      ...
      <div className="mt-6 flex flex-col gap-2 border-t border-border pt-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-xs text-subtle-foreground">
          &copy; {new Date().getFullYear()} Semper Admin. Open-source
          educational reference.
        </p>
        <span className="flex items-center gap-4">
          {/* Absent until a walkthrough URL is configured. */}
          {hasWalkthrough() ? (
            <a
